...
if [[ $AUDIT_MODE == 'local' ]]; then
  ...
  docker logs -f --tail 0 testcenter-backend > "$WORKING_DIR/backend.log" 2>&1 &
  docker logs -f --tail 0 testcenter-traefik > "$WORKING_DIR/traefik.log" 2>&1 &
  docker logs -f --tail 0 testcenter-db > "$WORKING_DIR/db.log" 2>&1 &
else
