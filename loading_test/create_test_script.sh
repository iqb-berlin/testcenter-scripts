...
if [[ $AUDIT_MODE == 'local' ]]; then
  ...
  docker cp loading_test/create_test_script.php testcenter-backend:/var/www/backend/loading_test/
  docker cp loading_test/loading_test.env testcenter-backend:/var/www/backend/loading_test/
