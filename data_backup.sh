...
backup_db() {
  printf "\nDB DUMP\n\n"
  printf "Starting %s ...\n" ${DB_CONTAINER_NAME}
  docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d ${DB_CONTAINER_NAME}
  printf "\n"
  ...
  docker exec $DB_CONTAINER_NAME sh -c \
    'exec mysqldump --verbose --all-databases --add-drop-database -uroot -p"$MYSQL_ROOT_PASSWORD"' \
    >"${BACKUP_DIR}/${DB_DUMP_NAME}"
  printf "'mysqldump' finished.\n\n"

  printf "Stopping %s ...\n" ${DB_CONTAINER_NAME}
  docker compose -f docker-compose.yml -f docker-compose.prod.yml down ${DB_CONTAINER_NAME}
  printf "%s stopped.\n\n" ${DB_CONTAINER_NAME}
}
