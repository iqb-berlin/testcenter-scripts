...
--data-raw '{"name":"massentest","password":""}'
)
...
curl -o /dev/null --location --silent --fail --show-error --header  "AuthToken: ""$AUTH_TOKEN" \
...
END=$(date +%s%N)
echo "$AUTH_TOKEN | Test $TEST_NR | $START | $END | " $(("$END" - "$START"))
